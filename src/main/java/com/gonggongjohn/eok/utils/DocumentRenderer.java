...
public class DocumentRenderer {
    ...
    private final Map<String, Predicate<String[]>> tokenMap;
    private final List<Page> pages = new ArrayList<>();
    private final List<Element> elements = new ArrayList<>();
    private final List<Integer> textureList;
    private final GuiLoading loadingScreen = new GuiLoading();
    ...
    public BufferedReader getResource(ResourceLocation location) throws IOException {
    ...
    private void appendText(String str) {
        ...
        for (int i = 0; i < str.length(); i++) {
            ...
            if (allStyles.indexOf(tmp.charAt(index)) == -1) {    // 样式代码无效，保持原样
                output.append(tmp.substring(0, 2));
                tmp = tmp.substring(2);
                continue;
            }
            // 正式开始处理样式
        ...
        List<String> lines = trimStringToWidthWithoutStyleMarks(output.toString(), this.width);
    ...
    public static abstract class Element {
        ...
        protected static class Image extends Element {
            ...
            private Image(File image, int windowWidth, int windowHeight) throws IOException {
