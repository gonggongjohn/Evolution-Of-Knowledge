...
public class GUIElementaryResearchTable extends GuiContainer {
    ...
    private final Slot invPaperInputSlot;
    private final Slot invPaperOutputSlot;
    private final boolean onResearchProgress = false;
    private List<Integer> lastFinList = new ArrayList<>();
    private List<Integer> finList = new ArrayList<>();
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
        ...
        if (finList != null && !finList.isEmpty() && lastFinList != null && !lastFinList.isEmpty()) {
            if (finList.size() > lastFinList.size()) {
                ...
                lastFinList = new ArrayList<>(finList);
            }
        }
    ...
    protected void actionPerformed(GuiButton button) throws IOException {
        ...
        if (button.id == 0) {
            ...
            if (compound == null) return;
                ...
                HashSet<Integer> relation = new HashSet<>();
                int result;
                for (int value : temp) {
                    relation.add(value);
                }
