...
public class GUISecondaryBlueprintTable extends GuiContainer {
    ...
    private int componentBracketCommonTextureV = 216;
    private int componentBracketChosenTextureU = 115;
    private int componentBracketChosenTextureV = 216;
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
        ...
        if(this.inventoryIn.getBlueprintSlot().getHasStack()) {
            ...
            if (!isBlueprintSlotUsing) {
                for (int i = 4; i <= 12; i++) {
                    this.buttonList.add(new ButtonSecondaryBlueprintTableCenter(i, calcBlueprintAnchorX(i - 4), calcBlueprintAnchorY(i - 4), 18, 18, this));
                }
    ...
    public void initGui() {
        ...
        this.buttonList.add(new ButtonSecondaryBlueprintTableComponent(3, offsetX + componentBracketOffsetX, offsetY + componentBracketOffsetY, this.componentBracketCommonTextureU, this.componentBracketCommonTextureV,
    ...
    protected void actionPerformed(GuiButton button) throws IOException {
        ...
        if(button.id == 1){
            if(isBlueprintSlotUsing){
                layerNum++;
                for(int i = 0; i < 9; i++){
                    ((ButtonSecondaryBlueprintTableCenter)this.buttonList.get(i + 4)).setContent(null);
                }
                ArrayList<Vec3i> indexList = this.onBuildingStructure.getLayerIndexList(layerNum);
                if(!indexList.isEmpty()) {
                    for (Vec3i index : indexList) {
        ...
        if(button.id == 3){
            ...
            btn.flipActive();
            if(btn.isActive()) activeItem = Items.IRON_INGOT;
            else activeItem = null;
        }
        if(button.id >= 4 && button.id <= 12 && activeItem != null){
